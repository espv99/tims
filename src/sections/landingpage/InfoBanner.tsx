...
const InfoBanner: React.FC<InfoBannerProps> = ({
  ...
  const stats = [
    { icon: <CalendarIcon />, label: 'Duración', value: duration },
    { icon: <BuildingIcon />, label: 'Modalidad', value: modality },
    { icon: <FreeIcon />, label: 'Costo', value: cost },
    { icon: <UserIcon />, label: 'Coordinador', value: coordinator },
  ];

  return (
    <section className="w-full my-10">
      <div className="text-center mb-8">
        <h2 className="text-2xl md:text-3xl font-bold text-[#001b55] dark:text-foreground mb-3">Información General</h2>
        <p className="text-muted-foreground max-w-3xl mx-auto text-sm md:text-base leading-relaxed">
          El Ingeniero/a en Tecnologías de la Información posee la capacidad de utilizar de manera eficiente y efectiva
          las tecnologías de la información, contribuyendo significativamente a mejorar el rendimiento de las
          organizaciones adaptándose a las tendencias tecnológicas locales, regionales y nacionales.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {stats.map((stat) => (
          <div
            key={stat.label}
            className="bg-card border border-border rounded-xl p-4 md:p-6 text-center hover:border-[#001b55]/30 hover:shadow-md transition-all duration-200 cursor-default"
          >
            <div className="flex justify-center mb-3 text-[#001b55] dark:text-primary">{stat.icon}</div>
